        ...
        <div className="rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 p-4">
          ...
          {isDraft && (
            <div className="flex gap-2 mt-4">
              ...
              <button onClick={async () => { const ok = await confirm({ title: t('stockCount.finalizeConfirm'), description: 'This will lock the count and apply variances to inventory. This cannot be undone.', variant: 'danger', confirmLabel: 'Finalize' }); if (ok) finalize.mutate(); }} disabled={finalize.isPending} className="px-4 py-2 rounded-lg bg-primary text-white text-sm font-medium disabled:opacity-50">{t('stockCount.finalize')}</button>
            </div>
          )}
