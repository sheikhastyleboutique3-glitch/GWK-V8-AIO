...
import { CreateFiscalPositionDto, UpdateFiscalPositionDto } from './fiscal-positions.dto';
...
export class FiscalPositionsController {
  ...
  create(@Body() dto: CreateFiscalPositionDto) { return this.svc.create(dto); }

  @Patch(':id') @Roles(Role.SUPER_ADMIN, Role.BRANCH_MANAGER)
  update(@Param('id', ParseIntPipe) id: number, @Body() dto: UpdateFiscalPositionDto) { return this.svc.update(id, dto); }
