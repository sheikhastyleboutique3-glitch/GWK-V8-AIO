...
async function main() {
  ...
  const branchGwkWestWalk = await prisma.branch.upsert({
    ...
    create: {
      ...
      address: 'West Walk, Al Sidr St, Lusail, Qatar', phone: '+974-4000-0002', cashFloat: 5000,
    },
  });
  ...
  const branchSbhWestWalk = await prisma.branch.upsert({
    ...
    create: {
      ...
      address: 'West Walk, Al Sidr St, Lusail, Qatar', phone: '+974-4000-0006', cashFloat: 3500,
    },
  });
  ...
  // ==========================================================================
  // V8: PRINTERS + KOT ROUTING (categories -> station printers)
  // ==========================================================================
  const printerKitchen = await prisma.printer.upsert({ where: { id: 1 }, update: {}, create: { id: 1, name: 'Hot Kitchen', connection: 'IP', ipAddress: '192.168.1.201', port: 9100, widthMm: 80, branchId: branchDoha.id } });
  const printerBarista = await prisma.printer.upsert({ where: { id: 2 }, update: {}, create: { id: 2, name: 'Barista Bar', connection: 'IP', ipAddress: '192.168.1.202', port: 9100, widthMm: 80, branchId: branchDoha.id } });
  const printerPastry  = await prisma.printer.upsert({ where: { id: 3 }, update: {}, create: { id: 3, name: 'Pastry / Bakery', connection: 'IP', ipAddress: '192.168.1.203', port: 9100, widthMm: 80, branchId: branchDoha.id } });
  await prisma.category.update({ where: { id: catHot.id },      data: { printerId: printerBarista.id } });
  await prisma.category.update({ where: { id: catCold.id },     data: { printerId: printerBarista.id } });
  await prisma.category.update({ where: { id: catPastries.id }, data: { printerId: printerPastry.id } });
  await prisma.category.update({ where: { id: catSweets.id },   data: { printerId: printerPastry.id } });
  await prisma.category.update({ where: { id: catBites.id },    data: { printerId: printerKitchen.id } });
  console.log('✅ Printers (3) + category KOT routing');

  // ==========================================================================
  // V8: PAYMENT METHOD CONFIGS + TAX RATE + POS TERMINAL (pos.config)
  // ==========================================================================
  await prisma.paymentMethodConfig.upsert({ where: { id: 1 }, update: {}, create: { id: 1, name: 'Cash', nameAr: 'نقدي', type: 'CASH', isCashCount: true, opensDrawer: true, sortOrder: 0 } });
  await prisma.paymentMethodConfig.upsert({ where: { id: 2 }, update: {}, create: { id: 2, name: 'Card', nameAr: 'بطاقة', type: 'BANK', isCashCount: false, sortOrder: 1 } });
  ...
  // ==========================================================================
  const floor = await prisma.restaurantFloor.upsert({ where: { id: 1 }, update: {}, create: { id: 1, branchId: branchDoha.id, name: 'Main Hall', nameAr: 'القاعة الرئيسية', background: '#f5f5f4' } });
  const tableLayout: [string, number, number, number, string][] = [
    ['T1', 2, 40, 40, 'ROUND'], ['T2', 2, 160, 40, 'ROUND'], ['T3', 4, 280, 40, 'SQUARE'],
    ['T4', 4, 40, 180, 'SQUARE'], ['T5', 6, 180, 180, 'RECTANGLE'], ['T6', 2, 360, 180, 'ROUND'],
    ['T7', 4, 40, 320, 'SQUARE'], ['T8', 8, 200, 320, 'RECTANGLE'],
  ];
  for (const [name, seats, posX, posY, shape] of tableLayout) {
    await prisma.restaurantTable.create({ data: { branchId: branchDoha.id, floorId: floor.id, name, seats, status: 'AVAILABLE', isActive: true, posX, posY, width: shape === 'RECTANGLE' ? 140 : 90, height: 90, shape: shape as any } });
  }
  console.log('✅ Floor "Main Hall" + 8 positioned tables');
